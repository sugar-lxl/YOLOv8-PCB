...
class Annotator:
    ...
    def text(self, xy, text, txt_color=(0, 0, 0), anchor="top", box_style=False):#(255, 255, 255)黑色字体
...
def plot_images(
    ...
    for i in range(bs):
        ...
        if paths:
            annotator.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(0, 0, 0))  # filenames
        if len(cls) > 0:
            idx = batch_idx == i
...
@plt_settings()
def plot_results(file="path/to/results.csv", dir="", segment=False, pose=False, classify=False, nmsfree=False, on_plot=None):
    """
    ...
    if classify:
        ...
        fig, ax = plt.subplots(2, 8, figsize=(18, 6), tight_layout=True)
        index = [1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14, 15, 16, 9, 10]
    else:
